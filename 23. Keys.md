...
| Aspect | Foreign Key |
|--------|-------------|
| **Purpose** | To create a link between two tables by referencing a primary key. |
| **Number per Table** | Zero, one, or multiple. A table can have multiple foreign keys. |
| **Mandatory Rule** | Values must exist in the referenced primary key OR be NULL. |
| **Main Benefit** | Enforces referential integrity and enables table relationships. |

Foreign keys are the "glue" that holds a relational database together. They transform a collection of separate tables into an interconnected web of meaningful data, ensuring that relationships between data remain valid and consistent.
